...
(function() {
  ...
  const FAST_SCROLL_DELTA_PIXELS = 120;
  ...
  SmoothScrollDownGesture.prototype.start = function(distance, callback) {
    ...
    var SCROLL_DELTA = FAST_SCROLL_DELTA_PIXELS;
  ...
  ScrollAction.prototype.getRemainingScrollDistance_ = function() {
    ...
    this.last_scroll_top = this.last_scroll_top || this.element_.scrollTop;
    const amount_scrolled = this.element_.scrollTop - this.last_scroll_top;

    this.remaining_distance = this.remaining_distance - amount_scrolled;

    console.log('remaining distance: ' + this.remaining_distance + ', amount scrolled: ' + amount_scrolled + ', element scroll top: ' + this.element_.scrollTop + ', last scroll top: ' + this.last_scroll_top);

    this.last_scroll_top = this.element_.scrollTop;

    return this.remaining_distance;
  }

  ScrollAction.prototype.start = function(opts) {
    opts = opts || {
      element: document.body
    };
    var opt_element = opts.element;
    if (opt_element === document.body) {
      opt_element.scrollTop = 1;
      if (opt_element.scrollTop !== 1) {
        opt_element = document.documentElement;
      }
    }
    ...
    this.remaining_distance = opts.distance;
    console.log('remaining distance set to ' + opts.distance);
  ...
  ScrollAction.prototype.onGestureComplete_ = function() {
    ...
    console.log('gesture took ' + Date.now() - this.time);
    this.time = Date.now();
  ...
  if (!window.chrome) {
    console.log('Chrome Scroll replaced with regular scroll');
